...
async def latest_episode_for_podcast(podcast_id: str) -> Optional[Episode]:
    return (
        await Episode.find(Episode.podcast_id == podcast_id)
        .sort("-episode_number")
        .limit(1)
        .first_or_none()
    )
...
async def episodes_for_podcast_by_ids_light(
    ...
    return (
        await Episode.find(
            Episode.podcast_id == podcast.id, In(Episode.id, episode_ids)
        )
        .project(EpisodeLightProjection)
        .sort("-episode_number")
        .to_list()
...
async def episodes_for_podcast_by_numbers_light(
    ...
    return (
        await Episode.find(
            ...
            In(Episode.episode_number, episode_numbers),
        )
        .project(EpisodeLightProjection)
        .sort("-episode_number")
        .to_list()
...
async def podcasts_for_ids(podcast_ids: list[str]) -> list[Podcast]:
    podcasts = (
        await Podcast.find(In(Podcast.id, podcast_ids)).sort("-last_updated").to_list()
    )
    return podcasts
...
async def episode_by_number(podcast_id: str, episode_number: int) -> Optional[Episode]:
    episode = await Episode.find_one(
        Episode.podcast_id == podcast_id, Episode.episode_number == episode_number
    )
    return episode


async def episode_lite_by_number(
...
async def save_image_for_podcast(podcast_id: str) -> Optional[bytes]:
    ...
    image = PodcastImage(
        podcast_id=podcast_id, image_url=podcast.image, content=resp.content
    )
    await image.save()
