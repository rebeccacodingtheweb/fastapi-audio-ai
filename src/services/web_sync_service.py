...
async def new_podcast_from_rss(
    channel: Element, url: str, etag: Optional[str]
) -> Optional[Podcast]:
    ...
    subtitle_node = or_element(
        channel.find("subtitle"), channel.find("itunes:subtitle", itunes_ns)
    )
    subtitle = None
    if subtitle_node is not None:
        subtitle = (
            subtitle_node.text.strip() if subtitle_node.text else ""
        ).strip() or None

    category_node = or_element(
        channel.find("category"), channel.find("itunes:category", itunes_ns)
    )
    category = None
    if category_node is not None:
        category = category_node.attrib.get("text", "").strip() or None

    image_node = or_element(
        channel.find("itunes:image", itunes_ns), channel.find("image")
    )
    image = None
    ...
    if website_url_node is not None:
        website_url = (
            website_url_node.text.strip().rstrip("/")
            if website_url_node is not None
            else ""
        ).strip() or None
...
async def add_episodes_for_new_podcast(podcast: Podcast, channel: Element):
    ...
    for idx, e in enumerate(items):
        ...
        try:
            tags = [
                t.strip().lower()
                for t in e.find("itunes:keywords", itunes_ns).text.split(",")
            ]
        except Exception:
            pass  # Yes, we will try/except/pass!

        explicit = str(
            e.find("itunes:explicit", itunes_ns) or "no"
...
def __get_feed_date_text(d):
    ...
    return "{day}, {day_of_month} {month} {year} {hour}:{minute}:{second} -0800".format(
        **data
    )
...
def or_element(
    primary: Optional[Element], fallback: Optional[Element]
) -> Optional[Element]:
