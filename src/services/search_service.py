...
async def search_episodes(searchable_words: set[str]) -> RawSearchResult:
    ...
    for podcast in podcasts:
        ...
        episodes = await podcast_service.episodes_for_podcast_by_numbers_light(
            podcast, episode_numbers
        )
...
async def build_index_core():
    ...
    print(
        f"<<< Search Engine: Indexing complete in {dt.total_seconds():,.0f} seconds.",
        flush=True,
    )
...
async def build_index_for_podcast(podcast: Podcast):
    ...
    episode_to_record_date: dict[int : datetime.datetime] = {
        r.episode_number: r.created_date for r in records
    }

    for ep in await podcast_service.episodes_for_podcast(podcast):
        if not await has_changed_contents(
            episode_to_record_date, ep.podcast_id, ep.episode_number
        ):
        ...
        transcript: Optional[EpisodeTranscript] = (
            await ai_service.full_transcript_for_episode(podcast.id, ep.episode_number)
        )
        ...
        record: Optional[SearchRecord] = await search_record_for_episode(
...
async def search_record_for_episode(
    podcast_id: str, episode_number: int
) -> Optional[SearchRecord]:
    record = await SearchRecord.find(
        SearchRecord.podcast_id == podcast_id,
        SearchRecord.episode_number == episode_number,
    ).first_or_none()
    return record
...
async def search_records_lite_for_podcast(podcast_id: str) -> list[SearchRecord]:
    record = (
        await SearchRecord.find(SearchRecord.podcast_id == podcast_id)
...
def load_search_model() -> bool:
    ...
    try:
        ...
        print(
            "WARNING: Search disabled. You must download the spacy model once to use search."
        )
        print(
            "Run: python -m spacy download en_core_web_lg in the virtual env for this project."
        )
        return False
