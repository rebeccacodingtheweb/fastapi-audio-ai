...
async def new_podcast_from_rss(
    podcast_rss: FeedParserDict, url: str, etag: Optional[str]
) -> Optional[Podcast]:
    ...
    podcast = Podcast(
        ...
        title=title.strip(),
        description=(
            feed.get("description") or feed.get("itunes:summary") or ""
        ).strip(),
        subtitle=(feed.get("subtitle") or feed.get("itunes:subtitle") or "").strip()
        or None,
...
async def add_episodes_for_new_podcast(podcast: Podcast, podcast_rss: FeedParserDict):
    ...
    for e in podcast_rss.entries:
        ...
        description = (
            ...
            or e.get("summary_detail", {}).get("value", "").strip()
            or None
        )
        if not description:
        ...
        summary = (
            e.get("summary", "").strip() or e.get("itunes_summary", "").strip() or None
        )
        ...
        explicit = str(e.get("itunes_explicit", "no") or "no").lower().strip() in {
            "yes",
            "true",
        }
