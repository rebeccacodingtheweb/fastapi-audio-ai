...
async def create_background_job(
    action: JobActions, podcast_id: str, episode_number: int
) -> BackgroundJob:
    job = BackgroundJob(
        action=action, podcast_id=podcast_id, episode_number=episode_number
    )
    await job.save()
...
async def worker_function():
    ...
    while True:
        ...
        try:
            ...
            print(
                f"Starting new job: {job.id}, {job.podcast_id} episode {job.episode_number}"
            )
        except Exception as x:
        ...
        try:
            episode = await podcast_service.episode_by_number(
                job.podcast_id, job.episode_number
            )
            if not episode:
            ...
            print(
                f"Error getting podcast details for job: j={job.id}, p={job.podcast_id}, e={job.episode_number}: {x}"
            )
        ...
        try:
            ...
            if job.action == JobActions.summarize:
                await ai_service.worker_summarize_episode(
                    job.podcast_id, job.episode_number
                )
            elif job.action == JobActions.transcribe:
                await ai_service.worker_transcribe_episode(
                    job.podcast_id, job.episode_number
                )
            elif job.action == JobActions.chat:
                await ai_service.worker_enable_chat_episode(
                    job.podcast_id, job.episode_number
                )
            else:
                raise Exception(f"What am I supposed to do with {job.action}?")
