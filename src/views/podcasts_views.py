...
async def image(podcast_id: str, ext: str):
    ...
    if not img_bytes:
        return webutils.return_error(
            f"No image found for podcast {podcast_id}", status_code=404
        )
    ...
    response = Response(
        content=img_bytes, media_type=f"image/{img_type}", status_code=200
    )
...
async def refresh_podcast(podcast_id: str):
    ...
    if podcast is None:
        return fastapi.responses.HTMLResponse(
            content="No podcast with that ID", status_code=404
        )
