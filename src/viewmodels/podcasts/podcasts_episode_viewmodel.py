...
class PodcastEpisodeViewModel(ViewModelBase):
    ...
    async def load_data(self):
        ...
        self.episode = await podcast_service.episode_by_number(
            self.podcast_id, self.episode_number
        )
        self.ai_summary = await ai_service.summary_for_episode(
            self.podcast_id, self.episode_number
        )
        self.transcript_sentences = (
            await transcript_service.transcript_text_for_episode(
                self.podcast_id, self.episode_number
            )
        )
        ...
        summary_url = (
            f"/podcasts/summary/{self.podcast_id}/episode/{self.episode_number}"
        )
