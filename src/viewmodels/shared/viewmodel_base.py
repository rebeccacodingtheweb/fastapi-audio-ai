...
class ViewModelBase:
    ...
    def __init__(self, request: Request, error: Optional[str] = None):
        ...
        self.user_id: Optional[bson.ObjectId] = cookie_auth.get_user_id_via_auth_cookie(
            request
        )
    ...
    async def load(self) -> bool:
        raise Exception(
            f"Model specific form load and validation not set on class {type(self).__name__}."
        )
