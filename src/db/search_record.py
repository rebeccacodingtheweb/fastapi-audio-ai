...
class SearchRecord(beanie.Document):
    ...
    class Settings:
        ...
        indexes = [
            pymongo.IndexModel(
                keys=[("keywords", pymongo.ASCENDING)], name="keywords_ascend"
            ),
            pymongo.IndexModel(
                keys=[("created_date", pymongo.ASCENDING)], name="created_date_ascend"
            ),
            pymongo.IndexModel(
                keys=[("episode_id", pymongo.DESCENDING)], name="episode_id_descend"
            ),
            pymongo.IndexModel(
                keys=[("episode_number", pymongo.DESCENDING)],
                name="episode_number_descend",
            ),
            pymongo.IndexModel(
                keys=[("episode_date", pymongo.DESCENDING)], name="episode_date_descend"
            ),
            pymongo.IndexModel(
                keys=[("podcast_id", pymongo.ASCENDING)], name="podcast_id_ascend"
            ),
            pymongo.IndexModel(
                keys=[
